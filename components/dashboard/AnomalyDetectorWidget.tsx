...
export const AnomalyDetectorWidget: React.FC = () => {
    ...
    const getSeverityColor = (severity: ShipmentAnomaly['severity']) => {
        switch (severity) {
            ...
            case 'LOW': return 'bg-chart-2/15 text-chart-2 border-chart-2/30';
    ...
    const getTypeIcon = (type: ShipmentAnomaly['type']) => {
        switch (type) {
            case 'ROUTE_MISMATCH': return <AlertTriangle className="w-4 h-4" />;
            case 'STALLED': return <AlertCircle className="w-4 h-4" />;
            case 'DELAY': return <TrendingUp className="w-4 h-4" />;
        ...
        <Card className="h-full border-border bg-card shadow-sm col-span-1 lg:col-span-2 flex flex-col xl:col-span-3 border-primary/20 relative overflow-hidden group">
            ...
            <CardHeader className="relative z-10 pb-4 flex flex-row items-start justify-between">
                <div>
                    ...
                    <CardDescription>
                        Monitoring {stats.totalAnalyzed} active shipments for irregularities using predictive heuristics.
                    </CardDescription>
                </div>
                <div className="flex items-center gap-3">
                    ...
                    {stats.criticalCount > 0 && (
                        <div className="flex flex-col items-end">
                            ...
                            <span className="text-2xl font-bold text-destructive leading-none mt-1">{stats.criticalCount}</span>
                        </div>
                    )}
                </div>
            </CardHeader>
                ...
                {anomalies.length === 0 ? (
                    <div className="flex flex-col items-center justify-center p-8 text-center h-48">
                        ...
                        <p className="text-xs text-muted-foreground mt-1 max-w-[250px]">
                            No anomalies detected across {stats.totalAnalyzed} active shipments. Operations are nominal.
                        </p>
                    </div>
                ) : (
                        ...
                        {anomalies.slice(0, 5).map((anomaly, idx) => (
                            ...
                            >
                                <div className="flex-1 space-y-1">
                                    <div className="flex items-center gap-2">
                                        <span className="font-mono text-sm font-bold cursor-pointer hover:underline text-primary" onClick={() => navigate(`/shipments/${anomaly.shipment.id}`)}>
                                            {anomaly.shipment.cn_number}
                                        </span>
                                        <Badge variant="outline" className={cn("px-2 py-0 h-5 text-[10px] font-semibold border flex items-center gap-1", getSeverityColor(anomaly.severity))}>
                                ...
                                <div className="flex items-center gap-4 w-full sm:w-auto mt-2 sm:mt-0 justify-between sm:justify-end">
                                    <div className="flex flex-col items-start sm:items-end">
                                        <span className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold">AI Confidence</span>
                                        <div className="flex items-center gap-1.5 mt-1">
                                            ...
                                            <span className="text-xs font-mono font-medium">{anomaly.confidenceScore}%</span>
                                        </div>
                                    </div>
                                    <Button size="sm" variant="secondary" className="h-8 text-xs shrink-0" onClick={() => navigate(`/shipments/${anomaly.shipment.id}`)}>
                                        Investigate
                                    </Button>
                                </div>
                            </div>

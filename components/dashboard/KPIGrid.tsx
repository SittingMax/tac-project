...
import { Box, Activity, CheckCircle, AlertTriangle, Percent, LucideIcon, TrendingUp, TrendingDown, DollarSign, Clock } from 'lucide-react';
 ...
 */
const AnimatedCounter = ({ value, displayValue, isCurrency }: { value: number; displayValue: string, isCurrency?: boolean }) => {
...
const KPICard = React.memo(({
  label, value, displayValue, icon: Icon, color, trend, trendLabel, sparklineData, path, isCurrency, onNavigate, index
}: KPICardProps) => {
        ...
        <div className="p-5 flex-1 flex flex-col relative z-10">
          <div className="flex justify-between items-start mb-4">
            <div className="flex items-center gap-3">
              ...
              <div className="text-sm text-muted-foreground font-medium line-clamp-1 pr-2">{label}</div>
            </div>
          </div>
              ...
              {trend !== undefined && (
                <div className="flex items-center gap-1 mt-2">
                  <span className={cn(
                    ...
                    isIncreasing && color !== 'destructive' ? "text-status-success bg-status-success/10" :
                      isDecreasing && color !== 'destructive' ? "text-status-error bg-status-error/10" :
                        isDecreasing && color === 'destructive' ? "text-status-success bg-status-success/10" : // fewer exceptions = good
...
export const KPIGrid: React.FC<KPIGridProps> = ({ isLoading: externalLoading = false }) => {
  ...
  return (
    <div data-testid="kpi-grid" className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-4 xl:grid-cols-7 gap-4 auto-rows-fr">

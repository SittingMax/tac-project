                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 pt-6 border-t border-border">
                    {['docketCharge', 'pickupCharge', 'packingCharge', 'handlingFee', 'insurance', 'fuelSurcharge'].map((field) => (
                        <div key={field} className="space-y-2">
                            ...
                            <Input type="number" {...form.register(field as 'docketCharge' | 'pickupCharge' | 'packingCharge' | 'handlingFee' | 'insurance' | 'fuelSurcharge')} className="text-right" />
                        </div>
                    ))}
                </div>
                ...
                <div className="mt-6 pt-4 border-t border-border">
                    <div className="flex justify-between items-center">
                        ...
                        <div className="relative w-40">
                            ...
                            <Input type="number" {...form.register('discount')} className="pl-10 text-right text-status-success font-bold bg-status-success/5 border-status-success/30 rounded-none" />
                        </div>
                    </div>
                </div>
            </Card>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-3">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                {gstApplicable && (
                                    <div className="flex items-center gap-1">
                                        <Input type="number" {...form.register('gstRate')} className="w-14 h-7 text-center text-xs" min="0" max="100" />
                        ...
                        <div className="flex justify-between items-center py-2 border-t border-border/50">
                            ...
                            <Input placeholder="₹0" type="number" {...form.register('advancePaid')} className="w-28 text-right h-8 text-sm" />
                        </div>
                    ...
                    <div className="flex flex-col items-end justify-center bg-primary/5 rounded-none p-6 border border-primary/20">
                        ...
                        <div className="text-xs text-muted-foreground mt-2">{gstApplicable ? 'Inclusive of GST' : 'GST not applicable'}</div>
                    </div>
                </div>
            </div>
        </div>

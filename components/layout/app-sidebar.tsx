  ...
  return (
    <Sidebar collapsible="icon" {...props} className="border-r-0">
      <SidebarHeader className="border-b border-sidebar-border px-4 py-3 h-16 flex items-center justify-center">
        <TacLogo size="sm" showSubtitle={!isCollapsed} collapsed={isCollapsed} linkTo="/dashboard" />
      </SidebarHeader>
      <SidebarContent>
        {navGroups.map((group) => {
              ...
              <SidebarGroupContent>
                <SidebarMenu>
                  {visibleItems.map((item) => {
                    ...
                    const badgeKey = (item as any).badgeKey; const badgeCount = badgeKey && badges ? (badges as any)[badgeKey] : 0

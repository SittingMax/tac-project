...
export const SectionHeader = ({
    ...
    <div className="flex items-center gap-3 mb-8 pb-4 border-b border-border/40">
        ...
        <h3 className="text-xs font-mono font-black border border-border px-2 py-0.5 uppercase tracking-widest text-foreground">{title}</h3>
    </div>
);
...
export const ToggleSwitch = ({
    ...
    <button
        ...
        onClick={onToggle}
    >
        <span className="text-[10px] font-mono uppercase tracking-widest text-foreground group-hover:text-primary transition-colors">{label}</span>
        <div
        ...
        >
            <div
                className={`absolute top-0 w-3.5 h-3 shadow-none transition-all duration-300 ${checked
                    ? 'right-0 bg-primary'
                    : 'left-0 bg-muted-foreground/30'
                    }`}
            ></div>
        </div>
    </button>

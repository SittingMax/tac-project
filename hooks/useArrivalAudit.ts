...
import { manifestKeys, type ManifestItemWithRelations, type ManifestWithRelations } from '@/hooks/useManifests';
...
export function useArrivalAudit() {
    ...
    const { data: items = [], isLoading: isLoadingItems, refetch: refetchItems } = useQuery({
        ...
        `)
            ...
            return (data || []).map((item: any) => ({
                ...item,
                status: item.shipment.status === 'RECEIVED_AT_DEST' ? 'SCANNED' :
                    item.shipment.status === 'EXCEPTION' ? 'EXCEPTION' : 'PENDING'
            })) as AuditItem[];
        },
        enabled: !!activeManifestId,
    });
    ...
    const processScan = useCallback(async (input: string, _source: ScanSource = ScanSource.MANUAL) => {
        if (!activeManifestId) {
        ...
        try {
            ...
            queryClient.setQueryData(manifestKeys.items(activeManifestId), (oldData: AuditItem[] | undefined) => {

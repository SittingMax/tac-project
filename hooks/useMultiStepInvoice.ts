...
export const steps = [
    { title: 'Basics', description: 'CN & Booking Info', fields: ['awb', 'bookingDate', 'paymentMode', 'invoiceNumber'] },
    {
        title: 'Parties', description: 'Consignor & Consignee', fields: [
            'customerId',
    ...
    { title: 'Cargo', description: 'Dims, Weight & Content', fields: ['contents', 'pieces', 'actualWeight', 'chargedWeight', 'dimL', 'dimB', 'dimH'] },
...
export function useMultiStepInvoice(_initialData?: unknown) {
    ...
    const form = useForm<InvoiceFormData>({
        ...
        defaultValues: {
            ...
            actualWeight: 0, chargedWeight: 0,
            ratePerKg: 120, docketCharge: 80,
            contents: 'Personal Effects',
            consignorState: 'Delhi', consigneeState: 'Manipur',
            consignorCity: 'New Delhi', consigneeCity: 'Imphal',
            declaredValue: 0, discount: 0, advancePaid: 0,
            insurance: 0, fuelSurcharge: 0, handlingFee: 0,
            packingCharge: 0, pickupCharge: 0,
    ...
    useEffect(() => {
        ...
        formValues.dimL, formValues.dimB, formValues.dimH, formValues.pieces,
        formValues.actualWeight, formValues.ratePerKg, formValues.chargedWeight,
        formValues.baseFreight, setValue, safeNum,
    ]);

    const subtotal = safeNum(formValues.baseFreight) + safeNum(formValues.docketCharge) +
        safeNum(formValues.pickupCharge) + safeNum(formValues.packingCharge) +
        safeNum(formValues.fuelSurcharge) + safeNum(formValues.handlingFee) +
        safeNum(formValues.insurance) - safeNum(formValues.discount);

    const tax = formValues.gstApplicable ? Math.round(subtotal * (safeNum(formValues.gstRate) / 100)) : 0;

...
export const manifestService = {
  ...
  async create(manifest: Omit<ManifestInsert, 'org_id' | 'manifest_no'>): Promise<Manifest> {
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...manifest,
        org_id: orgId,
        manifest_no: '', // Generated by DB trigger
      })
      .select()
      .single();
  ...
  async close(manifestId: string, staffId?: string): Promise<Manifest> {
    ...
    const { error } = await supabase.rpc('close_manifest_atomic', {
  ...
  async depart(manifestId: string, staffId?: string): Promise<Manifest> {
    ...
    // Use atomic RPC for data consistency
    const { error: rpcError } = await supabase.rpc('depart_manifest_atomic', {
      p_manifest_id: manifestId,
      p_staff_id: staffId as string,
      p_org_id: orgId,
    });
  ...
  async arrive(manifestId: string, staffId?: string): Promise<Manifest> {
    ...
    // Use atomic RPC for data consistency
    const { error: rpcError } = await supabase.rpc('arrive_manifest_atomic', {
      p_manifest_id: manifestId,
      p_staff_id: staffId as string,
      p_org_id: orgId,
    });
  ...
  async addShipmentByScan(
    ...
    // Use atomic DB RPC function for concurrency safety
    const { data, error } = await supabase.rpc('manifest_add_shipment_by_scan', {
      p_org_id: orgId,
      p_manifest_id: manifestId,
    ...
    // eslint-disable-next-line no-restricted-syntax -- RPC result type casting
  ...
  async removeShipmentById(
    ...
    // Use atomic DB RPC function for concurrency safety
    const { data, error } = await supabase.rpc('manifest_remove_item', {
      p_org_id: orgId,
      p_manifest_id: manifestId,
  ...
  async getScanLogs(manifestId: string): Promise<ManifestScanLog[]> {
    ...
    // Get scan logs using fresh types
    const { data, error } = await supabase
  ...
  async createEnterprise(params: CreateManifestParams): Promise<ManifestWithRelations> {
    ...
    const { data, error } = await supabase
      ...
      .insert({
        org_id: orgId,
        type: params.type,
        manifest_no: '', // 'manifest_no: \'\'' to satisfy TypeScript types.
  ...
  async recalculateTotals(manifestId: string): Promise<void> {
    ...
    const { error } = await supabase.rpc('manifest_update_totals', {
      p_manifest_id: manifestId,
    });

...
export const Analytics: React.FC = () => {
  ...
  const efficiencyData = useMemo(() => {
    ...
    return [
      ...
      { name: 'Exceptions', value: summary.exceptions, key: 'exceptions', color: 'var(--destructive)' },
    ];
  }, [summary]);
  ...
  if (isLoading) {
    return (
      <div className="space-y-12 animate-in fade-in duration-500">
        <PageHeader
          title="Telemetry"
          description="Real-time macroscopic telemetry."
        />
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-px bg-border/40 border-y border-border/40">
        <div className="bg-background p-8 flex flex-col justify-between group relative overflow-hidden transition-all hover:bg-muted/10">
          <div className="flex items-center justify-between mb-8">
            <span className="text-xs font-mono text-muted-foreground uppercase tracking-widest">Gross Volume</span>
            <Package className="w-4 h-4 text-primary opacity-50 group-hover:opacity-100 transition-opacity" />
          </div>
          <div className="text-5xl font-black tracking-tighter">{summary.total_shipments.toLocaleString()}</div>
        ...
        <div className="bg-background p-8 flex flex-col justify-between group relative overflow-hidden transition-all hover:bg-muted/10">
          <div className="flex items-center justify-between mb-8">
            <span className="text-xs font-mono text-muted-foreground uppercase tracking-widest">Delivery Rate</span>
          ...
          <div className={cn(
            ...
            {deliveryRate}<span className="text-2xl text-muted-foreground ml-1">%</span>
          </div>
          <div className="absolute bottom-0 left-0 w-full h-[2px] bg-primary transform scale-x-0 group-hover:scale-x-100 transition-transform origin-left duration-500"></div>
        </div>
        ...
        <div className="bg-background p-8 flex flex-col justify-between group relative overflow-hidden transition-all hover:bg-muted/10">
          <div className="flex items-center justify-between mb-8">
            <span className="text-xs font-mono text-muted-foreground uppercase tracking-widest">On Track</span>
            <CheckCircle className="w-4 h-4 text-primary opacity-50 group-hover:opacity-100 transition-opacity" />
          </div>
          <div className="text-5xl font-black tracking-tighter">{summary.on_track.toLocaleString()}</div>
          <div className="absolute bottom-0 left-0 w-full h-[2px] bg-primary transform scale-x-0 group-hover:scale-x-100 transition-transform origin-left duration-500"></div>
        </div>

        <div className="bg-background p-8 flex flex-col justify-between group relative overflow-hidden transition-all hover:bg-muted/10">
          <div className="flex items-center justify-between mb-8">
            <span className="text-xs font-mono text-destructive uppercase tracking-widest">Anomalies</span>
            <AlertTriangle className="w-4 h-4 text-destructive opacity-50 group-hover:opacity-100 transition-opacity" />
          </div>
          <div className="text-5xl font-black tracking-tighter text-destructive">{summary.exceptions.toLocaleString()}</div>
        ...
        <div className="space-y-6">
          <div className="flex items-end justify-between border-b border-border/40 pb-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground font-mono uppercase tracking-widest mt-1">6-Month historical macro-view</p>
            </div>
          </div>

          <div className="h-[350px] w-full pt-4">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={summary.monthly_data} margin={{ left: -20, right: 0, top: 0, bottom: 0 }}>
                <defs>
                ...
                <CartesianGrid vertical={false} strokeDasharray="2 2" stroke="var(--border)" opacity={0.4} />
                <XAxis
                  dataKey="month"
                  tickLine={false}
                  axisLine={false}
        ...
        <div className="space-y-6">
          <div className="flex items-end justify-between border-b border-border/40 pb-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground font-mono uppercase tracking-widest mt-1">Current state architecture</p>
            </div>
          </div>

          <div className="h-[350px] w-full pt-4">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={efficiencyData} layout="vertical" margin={{ left: 0, right: 0, top: 0, bottom: 0 }}>
                <CartesianGrid horizontal={true} vertical={false} strokeDasharray="2 2" stroke="var(--border)" opacity={0.4} />
                <XAxis
                  type="number"
                  tickLine={false}
                  axisLine={false}
                ...
                <YAxis
                  ...
                  axisLine={false}
                  width={100}
                  tick={{ fill: 'var(--foreground)', fontSize: 11, fontFamily: 'Inter', fontWeight: 600 }}
                />

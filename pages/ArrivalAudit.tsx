                    ...
                    {activeManifestId ? (
                        <Card className="p-6 rounded-none border border-primary/30 bg-primary/5 flex flex-col">
                            <div className="flex justify-between items-start mb-4">
                                <div>
                                    <div className="text-xs font-mono uppercase tracking-widest text-primary mb-1">Active Manifest</div>
                                    <div className="text-2xl font-black font-mono tracking-tighter text-foreground">{manifest?.manifest_no}</div>
                                </div>
                                <Button size="sm" variant="outline" className="rounded-none border-primary/20 text-primary hover:bg-primary/10" onClick={clearManifest}>
                            ...
                            <div className="grid grid-cols-2 gap-4 text-sm bg-background border border-border/50 p-4">
                                <div>
                                    <div className="text-[10px] text-muted-foreground uppercase tracking-widest mb-1">Origin Hub</div>
                                    <div className="font-bold">{manifest?.from_hub?.name}</div>
                                </div>
                                <div>
                                    <div className="text-[10px] text-muted-foreground uppercase tracking-widest mb-1">Destination Hub</div>
                        ...
                        <Card className="p-6 rounded-none border border-status-info/30 bg-status-info/5 flex flex-col border-dashed group">
                            <div className="flex items-center gap-4 text-status-info/80">
                                ...
                                <div>
                                    ...
                                    <div className="text-xs opacity-80 mt-1 uppercase tracking-widest font-mono">Scan a manifest barcode or enter its ID to begin tally.</div>
                                </div>
                            </div>
                        </Card>
                    )}
                            ...
                            <div className="flex-1 flex flex-col items-center justify-center p-8 bg-muted/5 relative">
                                ...
                                <div className="relative z-10 flex flex-col items-center">
                                    <div className="w-24 h-24 rounded-full border border-primary/20 bg-primary/5 flex items-center justify-center mb-6">
                                        <ScanLine className={`w-10 h-10 text-primary ${isProcessingScan ? 'animate-pulse' : ''}`} />
                                    </div>
                                    <h3 className="text-xl font-bold tracking-tight text-foreground uppercase">Ready to Scan</h3>
                                ...
                                <div className="w-full absolute bottom-0 left-0 p-4 bg-background border-t border-border z-20">
                                    <form onSubmit={handleScanSubmit} className="flex gap-0">
                                        ...
                                        />
                                        <Button type="submit" disabled={!currentCode || isProcessingScan} className="rounded-none font-mono uppercase tracking-widest text-xs px-6 h-12">
                                            Submit
                                        </Button>
                                    </form>
                                    {errorInput && (
                ...
                <div className="flex flex-col gap-4 min-h-0 opacity-100 transition-opacity duration-500">
                    {activeManifestId ? (
                        ...
                        <Card className="flex-1 rounded-none border border-border bg-background flex flex-col items-center justify-center p-8 text-center border-dashed">
                            ...
                            <h3 className="text-2xl font-black font-mono tracking-tighter text-foreground mb-2">Awaiting Manifest</h3>
                            <p className="text-muted-foreground text-sm max-w-[300px]">
                                Please scan a manifest barcode or enter a manifest ID to retrieve the list of expected shipments.
                            </p>
                        </Card>
                    )}
                </div>

...
const resolveHubCode = (value?: string | null): string => {
  ...
  const matchedHub = Object.values(HUBS).find(
    (hub) => hub.id === value || hub.code === value || hub.uuid === value
  );
  return matchedHub?.code ?? value;
};
  ...
  const result = shipmentData
    ? {
        shipment: {
          ...
          originHub: resolveHubCode(shipmentData.origin_hub?.code),
          destinationHub: resolveHubCode(shipmentData.destination_hub?.code),
          eta: '',
      ...
      <Card className="max-w-3xl mx-auto p-8">
        ...
        <div className="flex gap-2">
          <div className="relative flex-1">
            <Input
              placeholder="e.g., TAC2026..."
              className="h-12 text-lg"
              value={trackId}
              onChange={(e) => setTrackId(e.target.value)}
            />
          </div>
          <Button size="lg" className="px-8 h-12" onClick={handleTrack}>
            Track
          </Button>
        </div>
        {error && (
          <div className="text-status-error mt-4 text-left text-sm bg-status-error/10 p-3 rounded-md border border-status-error/20 inline-block">
            {error}
          </div>
        )}
      </Card>
      ...
      {result && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <Card className="lg:col-span-1 p-6">
            <div className="mb-8">
              <div className="text-xs text-muted-foreground mb-3">Current Status</div>
          ...
          <Card className="lg:col-span-2 relative overflow-hidden min-h-[400px] bg-background">
            ...
            <div className="absolute top-4 right-4 text-right">
              <div className="bg-background/70 backdrop-blur px-4 py-2 rounded-lg border border-border">
                <div className="text-xs text-muted-foreground">Estimated Arrival</div>
                <div className="text-lg font-bold text-foreground font-mono">
                  {result.shipment.eta || 'Not available'}
                </div>
              </div>
            </div>
